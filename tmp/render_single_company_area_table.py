...
if len(sys.argv) < 3:
    raise SystemExit("Usage: python render_single_company_area_table.py <company_code> <output_name.svg>")

company_code = sys.argv[1].strip()
output_name = sys.argv[2].strip()
output_path = ROOT / "docs" / output_name

with DATA_PATH.open("r", encoding="utf-8") as fh:
    rows = json.load(fh)

rows = [row for row in rows if str(row.get("company_code", "")).strip() == company_code]
rows.sort(key=lambda item: (item["area_name"], item["manager_name"]))

if not rows:
    raise SystemExit(f"No rows found for company {company_code}")
...
for row in rows:
    line_overflow += max(
        ...
        len(wrap_text(f'{row["area_name"]} ({row["area_code"]})' if row["area_code"] else row["area_name"], COLUMNS[1][2])),
        1,
    ) - 1
...
svg = [
    ...
    f'<text x="{PADDING_X + 22}" y="50" class="title">{escape(company_name)}</text>',
    f'<text x="{PADDING_X + 22}" y="76" class="subtitle">Áreas obtenidas desde SQL. La columna Encargado queda libre para completar.</text>',
]
...
for index, row in enumerate(rows):
    ...
    svg.append(f'<text x="{manager_x + 16}" y="{current_y}" class="cell"></text>')
