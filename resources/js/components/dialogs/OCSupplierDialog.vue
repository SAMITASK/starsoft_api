...
const { status, getStatusByValue } = useStatus();
      ...
      try {
        const res = await $api("/details-order", {
          ...
          onResponseError({ response }) {
            throw new Error(
              response._data?.message || "Error al obtener detalles"
            );
          },
        });
    ...
    <VCard>
      <div>
        <VToolbar color="primary">
          ...
          <VBtn icon variant="plain" @click="resetForm">
            <VIcon color="white" icon="ri-close-line" />
          </VBtn>
        </VToolbar>
      </div>
      ...
      <VCardText class="pt-5">
        ...
        <template v-else>
          <VForm>
            ...
            <VTable
              class="invoice-preview-table border text-high-emphasis overflow-hidden mt-6 mb-6"
            >
              <thead>
                <tr>
            ...
            <VRow>
              ...
              <VCol cols="12">
                <div class="d-flex justify-space-between align-center">
                  <template v-if="getStatusByValue(details.OC_CSITORD)">
                    <span
                      class="font-weight-bold d-flex align-center gap-x-2"
                      :class="`text-${
                        getStatusByValue(details.OC_CSITORD).color
                      }`"
                    >
                      <VIcon
                        :icon="getStatusByValue(details.OC_CSITORD).icon"
                        :color="getStatusByValue(details.OC_CSITORD).color"
                        size="20"
                      />

                      <template v-if="details.OC_CSITORD === '00'">
                        {{ getStatusByValue(details.OC_CSITORD).title }}
                        por {{ details.OC_CUSUARI }} el
                        {{ formatDate(details.OC_DFECDOC) }}
                      </template>

                      <template v-else>
                        {{ getStatusByValue(details.OC_CSITORD).title }}
                        por {{ details.NOMBRE_USUARIO }} el
                        {{ formatDate(details.FECHAHORA_CAMBIOESTADO) }}
                      </template>
                    </span>
                  </template>
                  <div>
